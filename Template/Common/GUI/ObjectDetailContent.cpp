...
void GUI::ObjectDetailContent::Render() {
    ...
    if (ImGui::BeginTabBar("##TabBar")) {
        if (m_pTargetObject->GetUsingShaderName() == "FrenselPhong") {
            if (ImGui::BeginTabItem("Assignment3GUI")) {
                const std::string &objName = m_pTargetObject->mObjectName;
                auto pShader = m_pTargetObject->m_pShader;
                auto &mixRatio = pShader->GetUniformValue<float>(objName,
                                                                 "FrenselPhongMixRatio");//OnlyReflect < 0, OnlyRefract > 1, 1 > mix >0
                const static std::vector<std::string> modeNames = {"ReflectOnly", "RefractOnly", "Frensel + Phong"};
                ...
                if (ImGui::BeginCombo("Modes", modeNames[intStatus].c_str())) {
                    for (int i = 0; i < 3; ++i) {
                        bool isSelected = (i == intStatus);
                        if (ImGui::Selectable(modeNames[i].c_str(), isSelected)) {
                            intStatus = i;
                            switch (intStatus) {
                                case 0: {
                                    mixRatio = -1.f;
                ...
                if (intStatus == 2) {
                    ImGui::SliderFloat("MixRatio", &mixRatio, 0.f, 1.f);
                }
                ImGui::EndTabItem();
            }
